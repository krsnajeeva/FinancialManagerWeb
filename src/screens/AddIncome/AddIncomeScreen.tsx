...
const AddIncomeScreen: React.FC = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const newId = await incomeFirebaseService.add({
        ...income,
        createdAt: new Date().toISOString(),
      });
      dispatch(addIncome({
      ...
      setLoading(false);
      alert('Failed to save income. Please check your connection.');
  ...
  return (
    ...
    >
      ...
      >
        <ScreenHeader title="Add Income" showBack={true} light rightIcon="dots-horizontal" />
      </div>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '10px' }}>
              ...
              >
                <Icon
                  name={INCOME_CATEGORIES.find(c => c.name === category)?.icon || 'tag'}
                  size={20}
                  color={INCOME_CATEGORIES.find(c => c.name === category)?.color || theme.accent}
                />
              </div>
          ...
          {focusedField === 'category' && (
            <div>
              ...
              <SelectionGrid
                ...
                selectedId={category}
                onSelect={(id) => {
                  const catName = INCOME_CATEGORIES.find(c => c.id === id)?.name || id;
                  setCategory(catName);
